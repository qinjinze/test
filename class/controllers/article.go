...
func (this *ArticleController) ShowArticleList() {
	//session判断
	fmt.Println("展示文章列表页----------------")
	userName := this.GetSession("remember")
	...
	fmt.Println("查询总记录数typeName:", typeName)
	fmt.Printf("查询总记录数typeName:=%T\n", typeName)
	...
	fmt.Println("articles=typeName", typeName, &articles)
	//传递数据
...
func (this *ArticleController) HandleAddArticle() {
	...
	logger.Info("articleName=", articleName, "content=", content)
	//2校验数据
	...
	fmt.Println("file, head, err=", file, head, err)
	defer file.Close()
	if err != nil {
		fmt.Println("处理文件上传err:", err)
	...
	//3.防止重名
	fileName := time.Now().Format("2006-01-02 15_04_05") + ext
	//存储
	...
	logger.Info("o.Insert=num, err:", num, err, &article)
